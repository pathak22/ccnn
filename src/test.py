...
def prepare():
    save_dir = '/mnt/a/pathak/fcn_mil_cache/visualized_output/2012val_best_tuned/'
    return save_dir
...
def compute_hist(net, save_dir, dataset):
    ...
    for fname in fnames_val[dataset]:
        ...
        h, _, _ = np.histogram2d(net.blobs['gt'].data[0, 0].flatten(),
                net.blobs['upscore-crf'].data[0,0].flatten(),
        ...
        #print ''
        # im = Image.fromarray(net.blobs['upscore-crf'].data[0,0].astype(np.uint8), mode='P')
...
def seg_tests(test_net, save_format, dataset, weights):
    ...
    for i in range(n_cl):
        iu[i] = hist[i, i] / (hist[i].sum() + hist[:, i].sum() - hist[i, i])
    print '>>>', datetime.now(), 'mean IU (out of 100)', np.nanmean(iu)*100
    iu2 = [ round(100*elem, 1) for elem in iu ]
    print '>>>', datetime.now(), 'per-class IU (out of 100)', iu2
...
seg_tests(test_net, save_format, dataset, weights)
